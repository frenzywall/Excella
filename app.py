...
# Dependency details for UI
DEPENDENCY_INFO = [
    {"name": "pandas", "import": "pandas", "required": True, "desc": "Data manipulation and analysis", "version": ">=2.2.3"},
    {"name": "openpyxl", "import": "openpyxl", "required": True, "desc": "Read/write .xlsx/.xlsm files", "version": ">=3.1.4"},
    {"name": "xlrd", "import": "xlrd", "required": True, "desc": "Read legacy .xls files", "version": ">=2.0.1"},
    {"name": "pyxlsb", "import": "pyxlsb", "required": False, "desc": "Read .xlsb files (optional)", "version": ">=1.0.9"},
    {"name": "fuzzywuzzy", "import": "fuzzywuzzy", "required": False, "desc": "Fuzzy string matching (optional)", "version": ">=0.18.0"},
    {"name": "python-Levenshtein", "import": "Levenshtein", "required": False, "desc": "Faster fuzzywuzzy (optional)", "version": ">=0.12.2"},
    {"name": "XlsxWriter", "import": "xlsxwriter", "required": False, "desc": "Alternative Excel writer (optional)", "version": ">=3.2.0"},
    {"name": "pyarrow", "import": "pyarrow", "required": False, "desc": "Faster data operations (optional)", "version": ">=16.1.0"},
    {"name": "pywin32", "import": "win32api", "required": False, "desc": "Enterprise Excel/COM support (Windows only, optional)", "version": ">=310"},
]
...
def check_package_installed(pkg_name, import_name=None, version_spec=None):
    ...
    try:
        ...
        return True  # If we can't get version, fallback to presence

class ExcelComparisonTool:
    def __init__(self, root):
        self.root = root
        self.root.title("Excel File Comparison & Signum Matching Tool")
    ...
    def update_mapping_display(self):
        ...
        if self.selected_target_columns and self.selected_replace_columns:
            ...
            for i in range(max_items):
                target = self.selected_target_columns[i][:12] + "..." if len(self.selected_target_columns[i]) > 12 else self.selected_target_columns[i]
                replace = self.selected_replace_columns[i][:12] + "..." if len(self.selected_replace_columns[i]) > 12 else self.selected_replace_columns[i]
        ...
        if data_source:
            data_short = data_source[:15] + "..." if len(data_source) > 15 else data_source
